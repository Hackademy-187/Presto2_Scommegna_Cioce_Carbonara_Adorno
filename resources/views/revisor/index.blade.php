                            ...
                            @if ($article_to_check->images->count())
                                @foreach ($article_to_check->images as $key => $image)
                                    <div class="col-6 col-md-4 mb-4 text-center">
                                        <img src="{{ $image->getUrl(500, 500) }}" class="img-fluid rounded shadow w-100 object-fit-cover" style="height: 250px;" alt="Immagine {{ $key + 1 }} dell'articolo '{{ $article_to_check->title }}'">
                                    </div>
                                @endforeach
                            @else
                                @for ($i = 0; $i < 6; $i++)
                    ...
                    <div class="col-12 col-md-4 d-flex flex-column justify-content-between"
                        ...
                        <div>
                            ...
                            <h3 class="mt-2"
                                ...
                                {{ __('ui.author') }}: {{ $article_to_check->user?->name ?? __('ui.anonymous') }}</h3>
                        ...
                        <div class="d-flex justify-content-between gap-2 mt-4">
                            <form action="{{ route('reject', ['article' => $article_to_check]) }}" method="POST"
                                ...
                                <button class="btn py-2.5 w-100 fw-bold rounded-pill"
                                    style="background-color: #ff5c5c; color: #1a1a1a; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; border: none;">{{ __('ui.reject') }}</button>
                            </form>
                            <form action="{{ route('accept', ['article' => $article_to_check]) }}" method="POST"
                                ...
                                @method('PATCH')
                                <button class="btn py-2.5 w-100 fw-bold rounded-pill"
                                    style="background-color: #d4a843; color: #1a1a1a; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; border: none;">{{ __('ui.accept') }}</button>
                            </form>
                        </div>
                    </div>
        ...
        @else
            <div class="container" style="padding-top: 100px; text-align: center;">
                ...
                <h1 class="fst-italic" style="color: #fff; font-size: 38px; margin: 20px 0;">
                    {{ __('ui.noArticlesToReview') }}</h1>
                <div class="main-border-button d-inline-block mt-3">
                    <a href="{{ route('homepage') }}" class="btn px-5 py-2.5 rounded-pill"
                        style="color: #d4a843; border: 1px solid #d4a843; text-decoration: none; text-transform: uppercase; font-size: 13px; font-weight: 600; background-color: transparent; transition: all 0.3s;">{{ __('ui.goHome') }}</a>
                </div>
            </div>
        @endif
